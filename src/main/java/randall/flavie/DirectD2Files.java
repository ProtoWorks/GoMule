                    ...
                    if (pItem.isUnique() && pItem.isJewel()) {
                        ...
                        for (int x = 0; x < pItem.getPropCollection().size(); x++) {
                            if (((D2Prop) (pItem.getPropCollection().get(x))).getPNum() == 197) {
                                matchStr[0] = D2TblFile.getString(D2TxtFile.SKILL_DESC.searchColumns("skilldesc",
                                                                                                     D2TxtFile.SKILLS.getRow(((D2Prop) (pItem.getPropCollection().get(x))).getPVals()[1])
                                                                                                                     .get("skilldesc")).get("str name"));
                                matchStr[1] = "Die";

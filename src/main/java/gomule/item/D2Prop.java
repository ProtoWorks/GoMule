    ...
    public String generateDisplay(int qFlag, int cLvl) {
        ...
        switch (funcN) {
            ...
            case (24):
                ...
                return "Level " + pVals[0] + " " + D2TblFile.getString(D2TxtFile.SKILL_DESC.searchColumns("skilldesc", D2TxtFile.SKILLS.getRow(pVals[1]).get("skilldesc"))
                                                                                           .get("str name")) + " " + oString;
            
            case (27):
                return "+" + pVals[1] + " to " + D2TblFile.getString(D2TxtFile.SKILL_DESC.searchColumns("skilldesc", D2TxtFile.SKILLS.getRow(pVals[0]).get("skilldesc"))
                                                                                         .get("str name")) + " " + D2TblFile.getString((D2TxtFile.SKILLS.getRow(D2TxtFile.SKILL_DESC.getRow(pVals[0])
                                                                                                                                                                                    .getRowNum())
                                                                                                                                                        .get("charclass")
                                                                                                                                                        .charAt(0) + "").toUpperCase() + D2TxtFile.SKILLS
                        .getRow(D2TxtFile.SKILL_DESC.getRow(pVals[0]).getRowNum())
                        .get("charclass")
                        .substring(1) + "Only");
            ...
            case (36):
                ...
                if (pVals.length == 4) {
                    ...
                    return "Adds " + Math.round(pVals[0] * ((double) pVals[2] / (double) pVals[3]) / 256) + " - " + Math.round(pVals[1] * ((double) pVals[2] / (double) pVals[3]) / 256) + " Poison Damage Over " + (int) Math
                            .floor(((double) pVals[2] / (double) pVals[3]) / 25.0) + " Secs (" + pVals[2] + " Frames)";

...
public class D2Character extends D2ItemListAdapter {
    ...
    private void resetStats() {
        ...
        if (hasMerc()) {
            ...
            mStats[18] = mStats[19] = mStats[20] = mStats[21] = (int) Math.floor((Integer.parseInt(mercHireCol.get("Resist")) + ((Double.parseDouble(mercHireCol.get("Resist/Lvl")) / (double) 4) * (getMercLevel() - Integer
                    .parseInt(mercHireCol.get("Level"))))));
    ...
    public boolean unmarkCharGrid(D2Item i) {
        ...
        switch (panel) {
            ...
            case BODY_STASH_CONTENT: // stash
                ...
                if ((col + width) > 6) { return false; }
                for (j = row; j < row + height; j++) {
                    for (k = col; k < col + width; k++) { iStashGrid[j][k] = false; }
                }
                break;
        }
        return true;
    ...
    public String fullDumpStr() {
        ...
        for (int x = 0; x < skillArr.size(); x = x + 1) {
            ...
            skillTrees[page - 1] = skillTrees[page - 1] + D2TblFile.getString(D2TxtFile.SKILL_DESC.searchColumns("skilldesc", ((D2TxtFileItemProperties) skillArr.get(x)).get("skilldesc"))
                                                                                                  .get("str name")) + ": " + initSkills[page - 1][skillCounter[page - 1]] + "/" + cSkills[page - 1][skillCounter[page - 1]] + "\n";
            skillCounter[page - 1]++;
        }
    ...
    private void modSetProps(D2Item sItem, int[] trackVal, int op) {
        ...
        for (int x = 0; x < sItem.getPropCollection().size(); x++) {
            switch (op) {
                case (1):
                    if ((((D2Prop) sItem.getPropCollection().get(x)).getQFlag() <= (trackVal[0]) && ((D2Prop) sItem.getPropCollection().get(x)).getQFlag() > 1 && ((D2Prop) sItem.getPropCollection()
                                                                                                                                                                                 .get(x)).getQFlag() < 7)) {
                    ...
                    } else if ((((D2Prop) sItem.getPropCollection().get(x)).getQFlag() <= (20 + trackVal[0]) && ((D2Prop) sItem.getPropCollection()
                                                                                                                               .get(x)).getQFlag() > 21 && ((D2Prop) sItem.getPropCollection()
                                                                                                                                                                          .get(x)).getQFlag() < 26)) {
                    ...
                    break;
                case (-1):
                    if ((((D2Prop) sItem.getPropCollection().get(x)).getQFlag() >= (trackVal[0] + 10) && ((D2Prop) sItem.getPropCollection()
                                                                                                                        .get(x)).getQFlag() > 11 && ((D2Prop) sItem.getPropCollection()
                                                                                                                                                                   .get(x)).getQFlag() < 17)) {
                        ((D2Prop) sItem.getPropCollection().get(x)).addCharMods(cStats, plSkill, (int) iCharLevel, -1, 1);
                        ((D2Prop) sItem.getPropCollection().get(x)).setQFlag(((D2Prop) sItem.getPropCollection().get(x)).getQFlag() - 10);
                    } else if ((((D2Prop) sItem.getPropCollection().get(x)).getQFlag() >= (30 + trackVal[0]) && ((D2Prop) sItem.getPropCollection()
                                                                                                                               .get(x)).getQFlag() > 31 && ((D2Prop) sItem.getPropCollection()
                                                                                                                                                                          .get(x)).getQFlag() < 36)) {
                        //Add display for partial set
                        ((D2Prop) sItem.getPropCollection().get(x)).setQFlag(((D2Prop) sItem.getPropCollection().get(x)).getQFlag() - 10);
    ...
    public long getMercDef() {
        ...
        return (long) (Math.floor((double) getMercInitDex() / (double) 4) + Integer.parseInt(mercHireCol.get("Defense")) + (Integer.parseInt(mercHireCol.get("Def/Lvl")) * (getMercLevel() - Integer.parseInt(
                mercHireCol.get("Level"))))) + cDef;
    }

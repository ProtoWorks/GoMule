...
public class MonsterTuple {
    ...
    public List<Object> deleteMiscDuplicated(List<Object> allTCS) {
        ...
        for (int x = 0; x < allTCS.size(); x = x + 1) {
            ...
            ProbTCRow dup = (ProbTCRow) allTCS.get(x);
            for (int y = 0; y < dup.getTC().size(); y = y + 1) {
                if (((String) dup.getTC().get(y)).indexOf("Runes") != -1 || ((String) dup.getTC().get(y)).indexOf("Act") != -1 || ((String) dup.getTC()
                                                                                                                                               .get(y)).indexOf("gld") != -1 || ((String) dup.getTC()
                                                                                                                                                                                             .get(y)).indexOf(
                        "Jewelry") != -1 || ((String) dup.getTC().get(y)).indexOf("Gem") != -1) {
                    dup.getTC().remove(dup.getTC().get(y));
                    dup.getProb().remove(y);
                    y = y - 1;
                }
    ...
    public void lookupBASETCReturnTrueMiscTCS(int nPlayers, int nplayersParty, double input, Item item, DCNew DC, int MF, boolean sevP) {
        ...
        if (mParent.getClassOfMon() == 2 || mParent.getClassOfMon() == 3 || this.getInitTC().indexOf("Champ") != -1 || this.getInitTC().indexOf("Super") != -1 || this.getInitTC()
                                                                                                                                                                      .indexOf("Unique") != -1 || ((String) getLastRow(
                miscTCS).getTC().get(0)).indexOf("Uitem") != -1) {
        ...
        if (((String) getLastRow(miscTCS).getTC().get(getLastRow(miscTCS).getTC().size() - 1)).indexOf("Runes") != -1 && ((String) getLastRow(miscTCS).getTC()
    ...
    public void lookupBASETCReturnATOMICTCS(int nPlayers, int nplayersParty, double input, boolean sevP) {
        ...
        if (picks < 0) {
            ...
            if (nPlayers > 1) {
                ...
                int probSum = ((ProbTCRow) allTCS.get(0)).getTotProb();
                if (noDrop != 0) {
                    noDrop = ((int) Math.floor(((double) probSum) / (BigDecimal.valueOf(((((double) 1) / (Math.pow((((double) noDrop) / ((double) (noDrop + probSum))), realnP)))))
                                                                               .subtract((BigDecimal.valueOf(1)))).doubleValue()));
                }
            }
